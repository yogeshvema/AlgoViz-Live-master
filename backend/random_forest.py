...
def rf(max_samples, dataset, num_est,bootstrap):
    plt.clf()
    if(dataset == "Dataset-1"): X,y=ds1,ds1_y
    if(dataset == "Dataset-2"): X,y=ds2,ds2_y
    if(dataset == "Dataset-3"): X,y=ds3,ds3_y
    if(bootstrap == False): max_samples = None
    clf = RandomForestClassifier(max_samples=max_samples, n_estimators=num_est, bootstrap=bootstrap)
    clf.fit(X, y)
